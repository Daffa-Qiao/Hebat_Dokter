...
<div class="container py-5">
    <div class="rounded-3 p-4 mb-4 text-white" style="background:linear-gradient(135deg,#ffc107,#ff8f00);">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h3 class="fw-bold mb-1"><i class="fas fa-utensils me-2"></i>Manajemen Menu Sehat</h3>
                <p class="mb-0 opacity-75">Kelola semua menu makanan sehat berdasarkan spesialisasi</p>
            </div>
            <button class="btn btn-light fw-semibold shadow-sm" data-bs-toggle="modal" data-bs-target="#createMenuModal">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="d-none d-md-table-cell">Kalori</th>
                    ...
                    <tbody>
                        @forelse($menus as $index => $menu)
                            <tr>
                                ...
                                <td>{{ $menu->title }}</td>
                                <td class="d-none d-md-table-cell">{{ $menu->specialization ?? 'Umum' }}</td>
                                <td class="d-none d-md-table-cell">{{ $menu->calories ? $menu->calories . ' kkal' : '-' }}</td>
                                <td class="d-none d-lg-table-cell">{{ $menu->doctor->name ?? 'Admin' }}</td>
                                <td class="text-nowrap">
                                    <button type="button" class="btn btn-sm btn-warning btn-edit-menu" title="Edit"
                                        data-action="{{ route('admin.healthy-menus.update', $menu) }}"
                                        data-id="{{ $menu->id }}"
                                        data-title="{{ e($menu->title) }}"
                                        data-category="{{ e($menu->category) }}"
                                        data-specialization="{{ $menu->specialization }}"
                                    ...
                                    <button type="button" class="btn btn-sm btn-danger btn-swal-delete" title="Hapus"
                                        data-action="{{ route('admin.healthy-menus.destroy', $menu) }}"
                        ...
                        @empty
                            <tr><td colspan="6" class="text-center py-4 text-muted">Belum ada menu sehat yang dibuat.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<form id="formDeleteMenu" method="POST" action="" style="display:none;">@csrf @method('DELETE')</form>

{{-- ===== MODAL TAMBAH MENU SEHAT ===== --}}
<div class="modal fade" id="createMenuModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.healthy-menus.store') }}" enctype="multipart/form-data" id="createMenuForm">
                @csrf
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form method="POST" action="" enctype="multipart/form-data" id="editMenuForm">
                @csrf @method('PUT')
                <input type="hidden" name="edit_action" id="editMenuAction" value="{{ old('edit_action') }}">
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Judul Menu</label>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="category" id="edit_menu_category" class="form-control">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Spesialisasi</label>
                            <input type="text" name="specialization" id="edit_menu_spec" class="form-control">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Kalori per Porsi</label>
                        <input type="number" name="calories" id="edit_menu_cal" class="form-control">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Deskripsi</label>
                        <textarea name="description" id="edit_menu_desc" class="form-control" rows="4"></textarea>
                    </div>
                    <div class="mb-3" id="editMenuImageWrap">
                        <label class="form-label fw-semibold">Gambar Saat Ini</label>
                        <div id="editMenuImagePreview"></div>
                    </div>
                    <div class="mb-3">
...
(function () {
    ...
    if (_ea) { document.getElementById('editMenuForm').action = _ea; new bootstrap.Modal(document.getElementById('editMenuModal')).show(); }
    else { new bootstrap.Modal(document.getElementById('createMenuModal')).show(); }
    @endif
    ...
    document.querySelectorAll('.btn-edit-menu').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('editMenuAction').value   = this.dataset.action;
            document.getElementById('edit_menu_title').value  = this.dataset.title;
            document.getElementById('edit_menu_category').value = this.dataset.category;
            document.getElementById('edit_menu_spec').value   = this.dataset.specialization;
            document.getElementById('edit_menu_cal').value    = this.dataset.calories;
            document.getElementById('edit_menu_desc').value   = this.dataset.description;
            const wrap = document.getElementById('editMenuImageWrap');
            const prev = document.getElementById('editMenuImagePreview');
    ...
    document.querySelectorAll('.btn-swal-delete').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (r.isConfirmed) { const f = document.getElementById('formDeleteMenu'); f.action = action; f.submit(); }
            });
        });
    });
})();
